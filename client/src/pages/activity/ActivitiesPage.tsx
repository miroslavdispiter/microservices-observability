...
import { ActivityStatusLabels, ActivityStatusColors } from "../../models/travel/Activity";

interface ActivitiesPageProps {
  isShared?: boolean;
}

export const ActivitiesPage = ({ isShared = false }: ActivitiesPageProps) => {
  ...
  const loadActivities = async () => {
    ...
    try {
      ...
      const data = await activityApi.getAll(actualId);
      setActivities(data);
    } catch (err: any) {
  ...
  const handleCreate = async (data: CreateActivityDto) => {
    if (!actualId) return;
    await activityApi.create(actualId, data);
    await loadActivities();
  };

  const handleUpdate = async (data: CreateActivityDto) => {
    if (!actualId || !editingActivity) return;
    await activityApi.update(actualId, editingActivity.id, data);
    await loadActivities();
  ...
  const handleDelete = async (activityId: number) => {
    ...
    try {
      await activityApi.delete(actualId, activityId);
      await loadActivities();
    } catch (err: any) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {!isShared && (
          <div className="flex items-center gap-2 text-sm mb-6">
            <button onClick={() => navigate("/travels")} className="text-indigo-600 hover:text-indigo-700 font-medium">
              My Travels
            </button>
            <svg className="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
            </svg>
            <button onClick={() => navigate(`/travels/${actualId}`)} className="text-indigo-600 hover:text-indigo-700 font-medium">
              {currentPlan?.title}
            </button>
            <svg className="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
          <div>
            ...
            <p className="text-gray-600 mt-2">
              {isShared ? "Daily activities for " : "Plan your daily activities for "}
              ...
              {!isViewOnly && (
                <div className="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-2xl border-2 border-indigo-200 p-6">
                  ...
                  <p className="text-sm text-gray-600">
                    Click on any date in the calendar to quickly add an activity for that day!
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="p-8">
              {isActivitiesLoading ? (
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500 mb-6">Start planning your daily activities!</p>
                  {!isViewOnly && (
                    <button
                      onClick={() => openCreateModal()}
                      className="bg-gradient-to-r from-indigo-500 to-purple-500 hover:from-indigo-600 hover:to-purple-600
                            ...
                            <div className="flex items-start justify-between gap-4">
                              <div className="flex-1">
                                <div className="flex items-start gap-3 mb-3">
                                  ...
                                  <div className="flex-1">
                                    <h4 className="text-lg font-bold text-gray-800 mb-1">{activity.name}</h4>
                                    ...
                                    <div className="flex items-center gap-3">
                                      <span className={`text-xs px-3 py-1 rounded-lg font-medium border ${ActivityStatusColors[activity.status]}`}>
