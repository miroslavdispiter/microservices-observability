...
import type { TravelPlan } from "../../types/TravelTypes";
...
export const TravelDetailsPage = () => {
  ...
  useEffect(() => {
    const loadPlan = async () => {
      ...
      try {
        setIsLoading(true);
        const res = await travelService.getById(parseInt(id));
        if (res.data.success) {
          setPlan(res.data.data);
        } else {
          setError(res.data.message || "Failed to load travel plan.");
        }
      } catch (err: any) {
        setError(err?.response?.data?.message || "Failed to load travel plan.");
      } finally {
        setIsLoading(false);
      }
    };
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-violet-50 via-indigo-50 to-purple-100">
        ...
        <div className="max-w-4xl mx-auto px-4 py-20 text-center">
          ...
          <p className="text-gray-600 mb-6">{error || "The requested travel plan does not exist."}</p>
          <button
