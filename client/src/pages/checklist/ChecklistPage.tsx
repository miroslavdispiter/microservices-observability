...
export const ChecklistPage = () => {
  ...
  const handleCreate = async (data: CreateChecklistItemDto) => {
    if (!id) return;
    await checklistApi.create(parseInt(id), data);
    await loadItems();
  };

  const handleUpdate = async (data: CreateChecklistItemDto) => {
    if (!id || !editingItem) return;
    await checklistApi.update(parseInt(id), editingItem.id, data);
    await loadItems();
    setEditingItem(null);
  };
  ...
  const handleToggle = async (itemId: number) => {
    ...
    try {
      await checklistApi.toggle(parseInt(id), itemId);
      await loadItems();
    } catch (err: any) {
  ...
  const handleDelete = async (itemId: number) => {
    ...
    try {
      await checklistApi.delete(parseInt(id), itemId);
      await loadItems();
    } catch (err: any) {
